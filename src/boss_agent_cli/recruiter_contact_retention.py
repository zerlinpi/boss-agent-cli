...
_PROTECTED_TEXT_PATTERNS: tuple[tuple[re.Pattern[str], str], ...] = (
    (re.compile(r"(?:婚姻状况|婚姻|marital\s*status)\s*[:：]?\s*(?:已婚|未婚|离异|丧偶|married|single|divorced|widowed)", re.I), "[婚姻状况已隔离]"),
    (re.compile(r"(?<![\u4e00-\u9fff])(?:已婚|未婚|离异|丧偶)(?![\u4e00-\u9fff])"), "[婚姻状况已隔离]"),
    (re.compile(r"(?:年龄|age)\s*[:：]?\s*\d{1,3}\s*(?:岁|years?\s*old)?", re.I), "[年龄已隔离]"),
    (re.compile(r"(?:性别|gender)\s*[:：]?\s*(?:男|女|male|female)", re.I), "[性别已隔离]"),
    (re.compile(r"(?:出生日期|生日|birth(?:day|\s*date)?)\s*[:：]?\s*\d{2,4}[-/.年]\d{1,2}(?:[-/.月]\d{1,2}日?)?", re.I), "[出生日期已隔离]"),
