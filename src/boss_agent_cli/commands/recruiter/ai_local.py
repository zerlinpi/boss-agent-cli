...
@click.pass_context
def configure_cmd(ctx: click.Context, job_key: str, jd_input: str, rubric_input: str | None, boss_job_id: str | None) -> None:
	...
	try:
		...
		return
	handle_output(ctx, "recruiter-ai-configure", record, hints={"next_actions": [
...
def evaluate_cmd(
	...
	try:
		jd_text, rubric = resolve_job(store, job_key=job_key, jd_input=jd_input, rubric_input=rubric_input)
		source = {"type": "local", "path": resume_input[1:]} if resume_input.startswith("@") else {
			"type": "local", "input": "inline",
		}
		...
		return
	handle_output(ctx, "recruiter-ai-evaluate", record, hints={"next_actions": [
...
def screen_cmd(
	...
	for path in sorted(resume_dir.glob(pattern))[:limit]:
		...
		try:
			record = evaluate_local(
				...
				jd_text=jd_text,
				rubric=rubric,
				resume_payload=read_json_input(f"@{path}"),
				job_key=job_key,
			...
			emit_ai_error(ctx, "recruiter-ai-screen", exc)
			return
	...
	if draft_top:
		try:
			...
			emit_ai_error(ctx, "recruiter-ai-screen", exc)
			return
	handle_output(ctx, "recruiter-ai-screen", {
		"job_key": job_key,
...
@click.pass_context
def batch_cmd(ctx: click.Context, jd_input: str, resume_dir: Path, pattern: str, job_key: str, top: int, limit: int) -> None:
...
def rank_cmd(ctx: click.Context, job_key: str, top: int) -> None:
	...
	handle_output(ctx, "recruiter-ai-rank", {
...
def report_cmd(ctx: click.Context, job_key: str, top: int) -> None:
	...
	handle_output(ctx, "recruiter-ai-report", RecruiterAIStore(ctx.obj["data_dir"]).report(job_key=job_key, top=top), hints={
		"next_actions": ["查看 top_candidates 的证据、风险和待确认问题", "boss hr ai mark --evaluation-id <id> --status interview"],
	})
...
def reply_cmd(ctx: click.Context, evaluation_id: str, conversation: str, intent: str) -> None:
	...
	try:
		...
		return
	handle_output(ctx, "recruiter-ai-reply", reply_record, hints={
		"next_actions": ["人工检查 draft.reply 后，再回到 BOSS 官方页面发送"],
	})
