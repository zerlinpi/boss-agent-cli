...
def test_save_crawl_config_recovers_invalid_files_and_preserves_unrelated_keys(tmp_path):
	...
	assert crawl._save_crawl_config(tmp_path, {"max_requests": 11, "chrome_path": None}) == {"max_requests": 11}
...
def test_require_crawl_capabilities_stops_at_each_policy_gate(tmp_path, monkeypatch):
	...
	def gate(_ctx: click.Context, capability: str) -> bool:
		calls.append(capability)
		return capability != "crawl"

	monkeypatch.setattr(crawl, "require_compliance_allowed", gate)
	...
	monkeypatch.setattr(
		...
		lambda _ctx, capability: (calls.append(capability), capability != "crawl-cdp")[1],
	)
	assert crawl._require_crawl_capabilities(ctx) is False
	...
	monkeypatch.setattr(
		crawl,
		"require_compliance_allowed",
		lambda _ctx, capability: (calls.append(capability), capability != "crawl-hook")[1],
	)
	assert crawl._require_crawl_capabilities(ctx, "screenshot-full") is False
	assert calls == ["crawl", "crawl-cdp", "crawl-hook"]

	calls.clear()
	monkeypatch.setattr(crawl, "require_compliance_allowed", lambda _ctx, capability: calls.append(capability) is None)
	assert crawl._require_crawl_capabilities(ctx, "none") is True
	assert calls == ["crawl", "crawl-cdp"]


def test_settings_from_context_uses_overrides_private_profile_and_zero_retries(tmp_path, monkeypatch):
	ctx = _ctx(
		tmp_path,
		config={
			"crawl": {
	...
	assert settings.profile_path == tmp_path / "crawl" / "chrome-profile"
	assert settings.chrome_path == "override-chrome"
	...
	assert settings.hook_profile == "screenshot-full"
	assert settings.hook_dir == tmp_path / "hooks"
	assert settings.max_requests == 9
	assert settings.max_details == 8
	assert settings.max_seconds == 70
	assert settings.max_retries == 0
	assert settings.operating_mode == "research"
...
def test_run_service_success_and_error_contracts(tmp_path, monkeypatch):
	...
	monkeypatch.setattr(crawl, "handle_output", lambda _ctx, _command, data, **kwargs: outputs.append((data, kwargs)))
	monkeypatch.setattr(crawl, "handle_error_output", lambda _ctx, _command, **kwargs: errors.append(kwargs))
...
def test_launch_background_resume_builds_fixed_command(tmp_path, monkeypatch):
	...
	assert command[-4:] == ["--from-queue", "--pages", "4", "--with-detail"]
...
def test_start_command_marks_created_run_stopped_when_background_launch_fails(tmp_path, monkeypatch):
	monkeypatch.setattr(crawl, "_require_crawl_capabilities", lambda ctx, hook_profile=None: True)
	monkeypatch.setattr(crawl, "operating_mode", lambda ctx: "research")
	monkeypatch.setattr(
		crawl,
	...
	assert json.loads(result.output)["error"]["code"] == "CRAWL_UNAVAILABLE"

	db_path = tmp_path / "cache" / "boss_agent.db"
	with sqlite3.connect(db_path) as connection:
		row = connection.execute("SELECT status, error FROM crawl_runs").fetchone()
	assert row is not None
	assert row[0] == "stopped"
	assert "spawn failed" in row[1]
...
def test_status_stop_and_results_commands_cover_success_and_missing(tmp_path, monkeypatch):
	...
	monkeypatch.setattr(crawl, "crawl_results", lambda cache, run_id, **kwargs: {"run_id": run_id, "filters": kwargs})
